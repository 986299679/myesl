/*
 * This file for test function `esl_recv_event_timed(&handle, 1000, 0, NULL)`:
 *
 * If no events, esl_recv_event_timed will block(no busy) 1000ms(1 second) and
 * return ESL_BREAK;
 * If receive a event during this 1000ms, it will return ESL_SUCCESS go to
 * `loop-brace-in` code block and run it.
 *
 * TODO: We need to test how to let this function return `ESL_GENERR`
 */
#include <esl.h>
